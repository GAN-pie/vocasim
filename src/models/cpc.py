...
# 
#     @classmethod
#     def get_autoregressive(cls, config):
#         return nn.GRU(
#             config['input_dim'],
#             config['ffn_dim'],
#             batch_first=True,
...
#         self.config = config
# 
#         self.g_enc = self.get_encoder(self.config)
#         self.g_ar = self.get_autoregressive(self.config)
#         self.Wk = self.get_projection(self.config)
# 
#         # TODO: initialize weights
# 
#     def forward(self, x: Tensor) -> Tensor:
...
#         K = self.config['steps']
# 
#         mask_pos = torch.arange(B) % self.config['noise_samples'] == 0
#         mask_neg = torch.arange(B) % self.config['noise_samples'] != 0
# 
#         # TODO :partitionate B into L * (noise_samples+1) and compute infoNCE independently
# 
#         z = self.g_enc(x)
#         _, c_t = self.g_ar(z[:, :-K])
# 
...
#         for k in range(1, K+1):
# 
#             proj = self.Wk[k-1](c_t)[..., None]
#             f_k = z[..., None, :] @ proj
# 
#             # not_k = torch.arange(preds.size(1)) != k
# 
#             pos = f_k[:, -k]
#             neg = torch.sum(f_k[mask_neg], dim=1)
# 
#             logsofmax_k += [
#                 -1.0 * F.log_softmax(f_k, dim=1)[]
#             ]
# 
#             logsofmax_k += [
#                 -1.0 * torch.log(pos / neg)
#             ]
# 
#         info_nce_k = torch.mean(logsofmax_k)
# 
# 
#         return torch.mean(Tensor(info_nce_k))
# 
